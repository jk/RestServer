...
class HttpMethods
{
     ...
     * resource with that URI. If a new resource is created, the origin server MUST inform the user agent via the
     * 201 (Created) response. If an existing resource is modified, either the 200 (OK) or 204 (No Content) response
     * codes SHOULD be sent to indicate successful completion of the request. If the resource could not be created or
     ...
     * the "*" request is only useful as a "ping" or "no-op" type of method; it does nothing beyond allowing the client
     * to test the capabilities of the server. For example, this can be used to test a proxy for HTTP/1.1 compliance
     * (or lack thereof).
     *
    ...
    /**
     * Get all HTTP methods where a client is allowed to send a request body
     *
     * @return array HTTP methods
     */
    public static function getMethodsWhereRequestBodyIsAllowed()
    {
        return array(
            self::POST,
            self::PUT
